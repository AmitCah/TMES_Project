...
public class ImageEncryptor {
    ...
    /**
     * Generates a pseudo-random diffusion mask for a specific pixel using bitwise operations.
     * The mask is dynamically influenced by the pixel's spatial coordinates, the current temporal iteration,
     * and the topological cryptographic keys (P and Q). It strictly outputs a 24-bit integer
     * to protect the alpha transparency channel during the XOR masking process.
     * * Complexity: O(1)
     *
     * @param x         The X coordinate of the pixel.
     * @param y         The Y coordinate of the pixel.
     * @param p         The Maximum Flow metric (Key P).
     * @param q         The Min-Cut Hash metric (Key Q).
     * @param iteration The current encryption round (temporal state).
     * @return A 24-bit integer mask to be safely XORed with the pixel's RGB value.
     */
    private static int generateMask(int x, int y, int p, int q, int iteration) {
        // The +1 prevents multiplying by zero on the first iteration or at coordinate (0,0).
        int xComponent = (x + iteration + 1) * p;
        int yComponent = (y + iteration + 1) * q;

        // 2. Combine using XOR, and add (x * y) to prevent linear vertical/horizontal lines.
        int hash = xComponent ^ yComponent ^ (x * y);

        // 3. Dynamic Bit-Shifting based on the Graph's Keys.
        // Instead of magic numbers, we shift the bits based on P and Q.
        // We use modulo 8 because there are 8 bits in a standard color channel (Red, Green, or Blue).
        hash ^= (hash << (p % 8));
        hash ^= (hash >>> (q % 8));

        // 4. Force into 24-bit RGB space (Zero out the Alpha transparency channel).
        // 0x00FFFFFF is just the standard hex representation of 24 ones in binary.
