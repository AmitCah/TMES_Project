...
public class ImageEncryptor {
     ...
     * Utilizes a non-linear bit-mixing function to guarantee the Avalanche Effect
     * and prevent structural/geometric artifacts in the ciphertext.
     */
    private static int generateMask(int x, int y, int p, int q, int iteration) {
        // 1. Combine the spatial and cryptographic inputs using large prime constants
        int hash = (x * 73856093) ^ (y * 19349663) ^ p ^ (q * 83492791) ^ iteration;

        // 2. Non-Linear Bit Mixing (MurmurHash3 Finalizer)
        // This violently cascades the bits across the integer, destroying any
        // linear geometric patterns generated by the X/Y coordinates.
        hash ^= hash >>> 16;
        hash *= 0x85ebca6b;
        hash ^= hash >>> 13;
        hash *= 0xc2b2ae35;
        ...
        // 3. Force the mask to fit within the 24-bit RGB space.
